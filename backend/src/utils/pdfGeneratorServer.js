...
const generateServerInvoicePDF = (sale, settings) => {
  const doc = new jsPDF({
  ...
  // 1. Page Background & Clean Double Border
  ...
  const saleDateStr = sale.date ? new Date(sale.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : new Date().toLocaleDateString('en-IN');
  doc.setFont('helvetica', 'normal');
  doc.text('Date     :', 119, 61);
  doc.text(saleDateStr, 145, 61);
  ...
  // Save to backend public invoices folder
  const invoicesDir = path.resolve(process.cwd(), 'public/invoices');
  if (!fs.existsSync(invoicesDir)) {
    fs.mkdirSync(invoicesDir, { recursive: true });
  }

  const filePath = path.join(invoicesDir, `${sale.billNumber}.pdf`);
  const pdfBuffer = doc.output('arraybuffer');
  fs.writeFileSync(filePath, Buffer.from(pdfBuffer));

  return `${sale.billNumber}.pdf`;
};
